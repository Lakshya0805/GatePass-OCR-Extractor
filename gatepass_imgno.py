...
with open("Output/GatePass.csv", "w", newline='') as csvfile:
    ...
    for file in file_list:
        if file.endswith('.jpg') or file.endswith('.png') or file.endswith('.jpeg'):
            ...
            #extracting txt from the processed img (use --psm6 for best result alternatively use psm3 or 4)
            text = pytesseract.image_to_string(thresh, config='--psm 6') 
            #regex for selecting only the gatepass
            gate = re.search(r'GatePass ?No\s*[:\-]?\s*([A-Z0-9]+)', text)
            # gatepass = gate.group(1)
            ...
            writer.writerow([filename, gate1])
